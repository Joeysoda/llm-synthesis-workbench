...
```mermaid
flowchart LR
    UI["模块化 React 平台<br/>127.0.0.1:5173"] -->|"HTTP + SSE"| GW["FastAPI 网关<br/>127.0.0.1:18000"]
    ...
    GW -->|"内部 HTTP"| KAQG["KAQG worker"]
    KAQG --> NEO[("Neo4j")]
    KAQG --> MQTT["Mosquitto"]
    GW --> CLEAN["Cleanlab 2.9.0<br/>本机特征与交叉验证"]
    GW --> PIPE["流程 DAG / 数据资产版本 / 血缘"]
    GW --> SYNTH["Synthea 内部 worker<br/>完全虚构医疗记录"]
...
| `/kaqg` | PDF、图谱摘要、分难度试题、证据和产物 |
| `/cleanlab` | 分类数据检查、分数、标签决策和清洗导出 |
| `/tasks` | 已完成产物与需要处理的任务 |
| `/settings` | 文本模型、视觉模型和本地组件诊断 |
| `/tools` | 五个工具的统一入口 |
| `/pipelines` | 模板化 DAG 流程编排与预检 |
| `/medical` | Synthea 医疗合成数据闭环 |
| `/datasets` | 数据集版本、质量门禁与发布 |
...
五个工具最后打开的项目分别保存在浏览器本地状态。页面切换后会主动清空旧任务状态，再从当前项目 API 恢复最近结果。
...
POST   /api/v2/kaqg/projects/{project_id}/jobs
POST   /api/v2/cleanlab/projects/{project_id}/jobs
PATCH  /api/v2/cleanlab/samples/{sample_id}/decision
GET    /api/v2/jobs
...
| `kaqg_build_graph` | medium | PDF 文本与图谱构建契约 |
| `kaqg_generate_evaluate` | medium | worker 生成、评分和产物整理 |
| `cleanlab_audit_dataset` | medium | 本机标签、异常和重复检查 |
...
- 上游固定在 commit `aa80de0082d5c217bbcb887ba386c2a85518c7d6`，由拉取脚本放入被忽略的 `upstream/kaqg`。
- worker 只处理当前任务目录中的 PDF；先检查文本层，再按页抽取实体和关系。
- Neo4j 和 Mosquitto 仅在 Compose 内部网络可达，不发布宿主机端口，也不挂载 Docker Socket。
- 所有 KAQG 任务串行执行。任务前清空固定图谱，完成后导出快照并再次清理。
- 模型密钥通过 worker 环境变量传入，不写入请求产物、SQLite 或日志。

### Cleanlab

- CSV/JSON/JSONL 解析、列映射、ID、数量和标签约束在运行前校验。
- 中文字符级 TF-IDF 使用 2～4 字符 n-gram；逻辑回归通过分层交叉验证产生样本外概率。
- TruncatedSVD 最多保留 64 维，Cleanlab 检查标签质量、异常和近重复。
- 标签检查固定单进程，避免 macOS 服务进程重复 spawn。
- 人工决策只更新当前标签和审核字段，不覆盖来源中的原标签。
...
Synthetic、Easy Dataset 与 KAQG 共用一个模型执行槽；无模型任务最多两个并发。任务切换时 SSE 订阅和旧页面状态都会清空。
...
- 不部署公网 Sites。
- 不修改 Easy Dataset 源码。
- 不做图谱编辑器、Playground、盲测、Hugging Face 上传、LLaMA Factory、其他 SynLogic 题型或模型训练。
- 上游接口固定在当前本机 commit；升级前必须重新运行集成测试。
- Synthea 只生成完全合成记录；平台不接收真实病例，不提供医疗诊断功能。
- 数据流程第一版复用既有网关与任务执行器，不引入 Airflow、Kubeflow 或第二套 WebUI。
